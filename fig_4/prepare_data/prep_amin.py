...
QUERY_PATH = (
    "/home/fleckj/projects/cellflow/data/datasets/AminPasca2023brx/v2/amin_full.h5ad"
)
MODEL_DIR = "/home/fleckj/projects/atlas/data/results/atlasv3_scanvi_scarches/v7/subregion_class/query_scarches_nolabs"

RESULTS_DIR = "/home/fleckj/projects/cellflow/results/amin_braun_mapping/"

PLOT_DIR = "/home/fleckj/projects/cellflow/plots/amin_braun_mapping/"
...
adata_ctrl = adata_q[adata_q.obs["condition"] == "CTRL"].copy()
adata_ctrl.obs = pd.DataFrame(
    {
        "condition": ["CTRL"] * adata_ctrl.shape[0],
        "batch": ["BraunCTRL"] * adata_ctrl.shape[0],
    }
)

adata_ctrl.write_h5ad(
    f"/home/fleckj/projects/cellflow/data/datasets/BraunCTRL/braun_2022_fetal_brain_subs_means_ctrl.h5ad"
)

adata_q.varm = dict()
adata_q.obs["batch"] = adata_q.obs["orig.ident"].astype(str).copy()
adata_q = adata_q[adata_q.obs["condition"] != "CTRL"].copy()
...
adata_q.write(
    f"/projects/site/pred/organoid-atlas/data/public_datasets/scg/brain_organoids/AminPasca2023brx/pasca_primary_screen_v1_common_hv2k_wknn.h5ad"
)

os.makedirs(f"{RESULTS_DIR}/amin_braun_scanvi", exist_ok=True)
mapper.save(f"{RESULTS_DIR}/amin_braun_scanvi/mapper.pkl")
...
#### Get wknn transfer ####
mapper = AtlasMapper.load(f"{RESULTS_DIR}/amin_braun_scanvi/mapper.pkl")
...
plt.savefig(f"{PLOT_DIR}/amin_braun_scanvi_umap_label_transfer.png")
...
plt.savefig(f"{PLOT_DIR}/amin_braun_scanvi_umap_leiden10.png")

adata_q.write(
    f"/projects/site/pred/organoid-atlas/data/public_datasets/scg/brain_organoids/AminPasca2023brx/pasca_primary_screen_v1_common_hv2k_wknn.h5ad"
)

os.makedirs(f"{PLOT_DIR}/scanvi_latent", exist_ok=True)

dims_to_plot = [(d, d + 1) for d in range(0, 20, 2)]
for dims in dims_to_plot:
    p = sc.pl.scatter(
        adata_q,
        color="region_pred_wknn",
        basis="scanvi_braun",
        show=False,
        components=dims,
    )
    plt.savefig(
        f"{PLOT_DIR}/scanvi_latent/amin_braun_scanvi_umap_region_dims_{dims[0]}_{dims[1]}.png"
    )
