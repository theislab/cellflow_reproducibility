...
# Set env vars
# Without this, torch and jax don't recognize the GPUs
...
echo "To access the dashboard, connect with from your local machine with 'ssh -L 1234:localhost:${dashboard_port} ${hosts[0]} -J shpc' and enter 'localhost:1234' in your browser."
...
then
    ...
    echo "Num gpus on head is:" $num_gpu
fi
...
echo "Starting RAY with launch command " $command_launch
...
# TODO: This could be done separately to not have to allocate all resources at once
for host in "${workers[@]}"
do
