...
if __name__ == "__main__":
    ...
    ablation_type = sys.argv[2]
    wandb_file = pd.read_csv(os.path.join(f"/lustre/groups/ml01/workspace/ot_perturbation/models/otfm/pbmc_new_donor_ablation_{ablation_type}", "wandb.csv"))
    wandb_id = pred_file.split("_")[0]
    if ablation_type == "batch_size":
        col = "training.batch_size"
    elif ablation_type == "epsilon":
        col = "model.epsilon"
    elif ablation_type == "unbalancedness":
        col = "model.tau_a"
    ablation_value = wandb_file[wandb_file["Name"]==wandb_id][col].values[0]

    complete_pred_file = os.path.join(f"/lustre/groups/ml01/workspace/ot_perturbation/models/otfm/pbmc_new_donor_ablation_{ablation_type}", pred_file)
    ...
    out_dir = f"/lustre/groups/ml01/workspace/ot_perturbation/data/pbmc/metrics_new_donor_{ablation_type}"
    ...
    out["ablation_value"] = ablation_value
