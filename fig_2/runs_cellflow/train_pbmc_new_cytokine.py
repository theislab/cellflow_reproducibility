...
def run(config):
    ...
    wandb_callback = cellflow.training.WandbLogger(project="cellflow_new_cytokine_new", out_dir="/home/icb/dominik.klein/tmp", config=config_dict)
    ...
    preds = cf.predict(adata=adata_ctrl, sample_rep="X_pca", condition_id_key="condition", covariate_data=covariate_data)
    ...
    for condition, array in preds.items():
        ...
        cond_orig = condition
        condition = condition + "_" + str(len(adata_pred.uns["donors_in_train"])) + "_" + str(len(adata_pred.uns["donors_in_train"]))
        ...
        pd.DataFrame.from_dict(out, columns=[f'{donor}_{cytokine_held_out}'], orient="index").to_csv(os.path.join(out_dir, f"{expr}_metrics.csv"))
