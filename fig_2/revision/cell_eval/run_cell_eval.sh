#!/bin/bash
# Run cell-eval metrics for all methods and donors in the new_donor use case.
...
for donor in $DONORS; do
    # Only run once per donor (use split 15 to determine cytokines, but identity doesn't use it)
    sbatch <<EOF
#!/bin/bash
#SBATCH -o ${LOG_DIR}/ce_identity_${donor}.out
#SBATCH -e ${LOG_DIR}/ce_identity_${donor}.err
#SBATCH -J ce_id_${donor}
#SBATCH -p cpu_p
