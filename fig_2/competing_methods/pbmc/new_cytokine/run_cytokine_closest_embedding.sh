...
for split in "${splits[@]}"; do
    for index in {0..42}; do
        ...
        sbatch <<EOF
#!/bin/bash
#SBATCH -o pbmc_emb_"$split"_"$index".out
#SBATCH -e pbmc_emb_"$split"_"$index".err
#SBATCH -J pbmc_emb_"$split"_"$index"
#SBATCH -p gpu_p
...
python new_cytokine_closest_embedding.py "$split" "$index"
EOF
    done
done
