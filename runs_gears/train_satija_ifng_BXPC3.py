...
def run(cfg: DictConfig):
    ...
    pathway = cfg.dataset.pathway
    cell_type = cfg.dataset.cell_type
    with open(cfg.dataset.custom_split, "rb") as f:
        custom_split_dict = pickle.load(f)
    ...
    assert all([gene not in genes_not_in_GO_graph for gene in custom_split_dict['train']])
    assert all([gene not in genes_not_in_GO_graph for gene in custom_split_dict['test']])
    ...
    for i in range(training_blocks):
        ...
        if test_data == None:
            ...
            testset = load_data(adata_test, cfg, skip_cond=genes_not_in_GO_graph, deg=deg)
            oodset = load_data(adata_ood, cfg, skip_cond=genes_not_in_GO_graph, deg=deg)
            test_data = {
                'test': testset,
                'ood': oodset
            }
