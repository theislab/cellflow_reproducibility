...
for cond in adata_ood.obs["perturbation_condition"].cat.categories:
    ...
    adata_pred_ood = adata_ood[adata_ood.obs["perturbation_condition"] == f"{cell_line}_{pathway}_NT"]
...
pd.DataFrame.from_dict(ood_metrics_encoded).to_csv(os.path.join(output_dir, f"identity_ood_metrics_encoded_{split}.csv"))
pd.DataFrame.from_dict(ood_metrics_decoded).to_csv(os.path.join(output_dir, f"identity_ood_metrics_decoded_{split}.csv"))
pd.DataFrame.from_dict(deg_ood_metrics).to_csv(os.path.join(output_dir, f"identity_ood_metrics_ood_{split}.csv"))
