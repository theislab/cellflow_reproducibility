...
def run_otfm_bsub(config):
    ...
    # Run CPA
    cmd = [
        "bash /pmount/projects/site/pred/organoid-atlas/USERS/bobrovsd/scripts/bnchmrk/cpa_sub.sh"
    ]
    ...
    with open(run_config, "w") as f:
        f.write(yaml.dump(config, sort_keys=False))
    cmd = [
        "bash /pmount/projects/site/pred/organoid-atlas/USERS/bobrovsd/scripts/bnchmrk/bnchmrk_eval_sub.sh"
    ]
    cmd.append(run_config)
    cmd.append(f'"{bsub_str}"')
    cmd = " ".join(cmd)
