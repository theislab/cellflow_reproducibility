...
if __name__ == "__main__":
    ...
    out_dir = "/lustre/groups/ml01/workspace/ot_perturbation/models/identity/zebrafish/single_condition_displacement_vector_combination"
    ood_conds = adata[adata.obs["gene_target"]!="control_control"].obs["condition"].unique()
    ood_cond_results = {}

    # first start with combination where both singletons are observed
    # ood_conds_additive = ["tfap2a_foxd3_36", "tfap2a_foxd3_48", "tfap2a_foxd3_72"]


    ood_conds_gt1_observed = ['cdx4_cdx1a_36','tbx16_msgn1_24','tbx16_tbx16l_24','tbx16_msgn1_36','cdx4_cdx1a_24','tbx16_tbx16l_18','cdx4_cdx1a_18','tbx16_tbx16l_36','tbx16_msgn1_18']
    
    for ood_cond in ood_conds_gt1_observed:
        print(ood_cond)
        ...
        gt1 = ood_cond.split("_")[0]
        
        adata_ood_pred = adata[(adata.obs["gene_target_1"]==gt1) & (adata.obs["gene_target_2"]=="control") & (adata.obs["timepoint"]==tp)]
        ...
        pd.DataFrame.from_dict(ood_cond_results[ood_cond], columns=[ood_cond], orient="index").to_csv(os.path.join(out_dir, f"{ood_cond}_one_gene_target_model.csv"))
