...
def plot_embedding(
    ...
    if show_lines:
        for i in range(len(emb)):
            if col_dict is None:
                ax.plot([0, emb[i, 0]], [0, emb[i, 1]], alpha=line_transparency, linewidth=line_width, c=None)
            else:
                ax.plot(
                    [0, emb[i, 0]], [0, emb[i, 1]], alpha=line_transparency, linewidth=line_width, c=col_dict[labels[i]]
                )
    ...
    if show_text and labels is not None:
        ...
        for label in unique_labels:
            ...
            texts.append(ax.text(np.mean(emb[idx_label, 0]), np.mean(emb[idx_label, 1]), label, fontsize=fontsize))

        adjust_text(texts, arrowprops=dict(arrowstyle="-", color="black", lw=0.1), ax=ax)
...
###############################
# everything below is copied from https://github.com/leotac/joypy/blob/master/joypy/joyplot.py, but we adapted it to plot the log-density
