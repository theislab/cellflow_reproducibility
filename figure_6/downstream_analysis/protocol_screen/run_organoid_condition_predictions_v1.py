...
RESULTS_DIR = f"{RESULTS_DIR}/{TRIAL_NAME}/new_predictions"
os.makedirs(RESULTS_DIR, exist_ok=True)

MODEL_DIR = (
    "/home/fleckj/projects/cellflow/results/train_eval_organoids_common/{TRIAL_NAME}/"
)
...
all_mols = ["SHH", "SAG", "FGF2", "FGF8", "BMP4", "BMP7", "LDN", "CHIR", "RA"]
...
combs1 = list(itertools.combinations(all_mols, 1))
combs2 = list(itertools.combinations(all_mols, 2))
combs3 = list(itertools.combinations(all_mols, 3))
all_trial_combs = combs1 + combs2 + combs3

mol2concs_data = {}
for mol in all_mols:
    mol_concs = adata.obs[mol + "_conc"].unique().tolist()
    mol_concs = np.array(_flatten_list([c.split("_") for c in mol_concs]), dtype=float)
    mol_concs = np.unique(mol_concs[mol_concs > 0])
    mol2concs_data[mol] = mol_concs
...
for mol_comb in all_trial_combs:
    ...
    comb_timings = {mol: list(timings.keys()) for mol in mol_comb}
    timing_combs = list(itertools.product(*comb_timings.values()))

    for concs in conc_combs:
        for timing_keys in timing_combs:
            cond_name = []
            mol_reps = {}
