...
def run(config):
    ...
    cellflow.preprocessing.centered_pca(adata_train, n_comps=100, keep_centered_data=False)
    cellflow.preprocessing.project_pca(query_adata=adata_ood_perturbed, ref_adata=adata_train)
    ...
    cf.prepare_model(
        ...
        condition_mode=config_dict["model"]["condition_mode"],
    ...
    wandb_callback = cellflow.training.WandbLogger(project="cellflow_pbmc_new_donor_revisions", out_dir="/home/icb/dominik.klein/tmp", config=config_dict)
    ...
    for i in range(len(covariate_data)):
        ...
        for condition, array in preds.items():
            ...
            cellflow.preprocessing.reconstruct_pca(query_adata=adata_pred, ref_adata=adata_train, use_rep="X_pca", layers_key_added = "X_recon")
            cond_orig = condition
