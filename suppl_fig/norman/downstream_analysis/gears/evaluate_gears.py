...
basepath = "/lustre/groups/ml01/workspace/leander.dony/projects/cellflow/241106_gears/evaluate_gears/output/norman/"

adata_all = sc.read(os.path.join(basepath, "adata_all.h5ad"))
...
for ood_setting, res in results.items():
    adata_ood = sc.read(os.path.join(basepath, f"adatas_{scenario}", f"norman_{scenario}_seed{ood_split}_test_{ood_setting}_truth.h5ad"))
    adata_pred_ood = sc.read(os.path.join(basepath, f"adatas_{scenario}", f"norman_{scenario}_seed{ood_split}_test_{ood_setting}_pred.h5ad"))
