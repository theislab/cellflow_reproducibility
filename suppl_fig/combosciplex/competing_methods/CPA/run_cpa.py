...
def get_hash():
    ...
    seed = int(current_time * 1000)  # Convert to milliseconds for more granularity
...
def run(config):
    ...
    model.train(
        ...
        save_path=f"/lustre/groups/ml01/workspace/ot_perturbation/models/cpa/combosciplex_count/{split}",
    )
    ...
    adata_pred_ood.layers["CPA_pred"] = adata_pred_ood.obsm["CPA_pred"].copy()
    if config.generic_params.use_counts:
        sc.pp.normalize_total(adata_pred_ood, layer="CPA_pred", target_sum=10_000)
        sc.pp.log1p(adata_pred_ood, layer="CPA_pred")
    
    r2_values = []
    for cond in adata_ood.obs["condition"].cat.categories:
        if cond == "control":
            continue
        cond_true = adata_ood[adata_ood.obs["condition"] == cond].X.toarray().mean(axis=0).squeeze()
        cond_pred = adata_pred_ood[adata_pred_ood.obs["condition"] == cond].layers["CPA_pred"].mean(axis=0).squeeze()
        r2_values.append(r2_score(cond_true, cond_pred))

    target_value = np.mean(r2_values)

    hash_val = get_hash()

    output_dir = "/lustre/groups/ml01/workspace/ot_perturbation/hparam/cpa_combosciplex"
    # Save the return value
    return_file = os.path.join(output_dir, f"results/{hash_val}_return_value.json")
    with open(return_file, "w") as f:
        json.dump(target_value, f)

    # Save the configuration
    config_file = os.path.join(output_dir, f"config/{hash_val}_config.json")
